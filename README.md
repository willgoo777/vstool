...
👉 **直接下这个**：[**vstool-nuitka-standalone.zip**（最新版）](https://github.com/willgoo777/vstool/releases/latest/download/vstool-nuitka-standalone.zip)

### 使用步骤

1. 下载上面的 `vstool-nuitka-standalone.zip`（约 200 MB）
2. 右键 → **全部解压**
3. 进入解压出来的 `vstool-standalone\` 文件夹
4. **双击里面的 `vstool.exe`**（不是 zip 外层的名字，是文件夹**内**的那个）

> ⚠️ 整个 `vstool-standalone\` 文件夹要一起拷贝，里面的 DLL 和 vstool.exe 是配套的。
...
- `vstool-nuitka.exe` — 同样是 Nuitka 编译，但塞进单 exe，首次启动会解压到 `%TEMP%`
- `vstool-pyinstaller.exe` — 传统 PyInstaller 打包，**在装 Uniaccess / 奇安信 / 深信服等国产 EDR 的电脑上几乎必拦**，只适合个人电脑
