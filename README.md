...
去 [Releases 页面](https://github.com/willgoo777/vstool/releases) 选最新版本，根据**你公司终端管控（如 Uniaccess / 360 / 火绒）的严苛程度**，按下面顺序选：

| 产物 | 推荐场景 | 体积 | 启动速度 | EDR 误报率 |
|---|---|---|---|---|
| `vstool-nuitka-standalone.zip` ⭐ | **被 Uniaccess 等企业 EDR 拦过的首选** | ~400 MB（解压后） | 极快 | 极低 |
| `vstool-nuitka.exe` | 想要单文件、可接受首次解压 | ~200 MB | 中（首次 5-10s） | 低 |
| `vstool-pyinstaller.exe` | 个人电脑 / 无终端管控环境 | ~250 MB | 中（首次 5-10s） | 中-高 |

### 三种产物的区别

- **Nuitka standalone（`.zip` 文件夹）**：Python 真正编译成 C，输出是**真正的原生 PE 二进制**，不解压、启动直接跑、AV/EDR 看着跟普通商业软件无异。解压 zip 后整个 `vstool-standalone/` 文件夹要一起拷，**双击里面的 `vstool.exe`**。
- **Nuitka onefile（`-nuitka.exe`）**：同样是 Nuitka 编译，但塞进单个 exe，首次启动解压到 `%TEMP%`。比 standalone 方便分发，比 PyInstaller 安全。
- **PyInstaller（`-pyinstaller.exe`）**：传统 Python 打包方式，bootloader 解压 Python 运行时。某些企业 EDR（如联软 Uniaccess、奇安信、深信服）会因 bootloader 行为模式将其标记为"可疑"。

### 都跑不动？

如果三个都被你公司 EDR 拦了，看 [`IT_WHITELIST_APPLICATION.md`](IT_WHITELIST_APPLICATION.md) 用我们准备的申请模板向公司 IT 申请白名单。所有产物都从 GitHub Actions 在**官方 windows-latest runner** 上构建，构建日志全公开可审计。

### 开发版

[Actions 页面](https://github.com/willgoo777/vstool/actions) → 最新一次绿色 ✓ 的 build → Artifacts 区域下载（90 天内有效，需要 GitHub 登录）。
