...
👉 **直接下这个**：[**vstool.zip**（最新版）](https://github.com/willgoo777/vstool/releases/latest/download/vstool.zip)

### 使用步骤

1. 下载上面的 `vstool.zip`（约 200 MB）
2. 右键 → **全部解压**
3. 进入解压出来的 `vstool\` 文件夹
4. **双击里面的 `vstool.exe`**

> ⚠️ 整个 `vstool\` 文件夹要一起拷贝，里面的 DLL 和 vstool.exe 是配套的。

### 为什么选这个

这是用 Nuitka 编译的**原生 PE 二进制**——Python 被真正编译成 C 再编成机器码，跟普通商业 Windows 软件结构一致。已在装有 **Uniaccess（联软）终端管控**的公司电脑上实测通过，不被拦截。

### 备用方案

如果上面这个有问题（极少数情况），Release 页面里还有：

- `vstool-onefile.exe` — 同样是 Nuitka 编译，但塞进单 exe，首次启动会解压到 `%TEMP%`

### 都跑不动？申请白名单

把 [`IT_WHITELIST_APPLICATION.md`](IT_WHITELIST_APPLICATION.md) 发给 IT，里面有完整 hash 和安全性说明。

### 想要每次提交的开发版

[Actions 页面](https://github.com/willgoo777/vstool/actions) → 最新一次绿色 ✓ 的 build → 下方 Artifacts 区域下载（90 天内有效，需要 GitHub 登录）。
...
- `对比有差异/` — 真有差异的对比产物，直接打开就能看。
- `对比无差异/` — 跑通了但没差异的产物（保留以便复核）。
- `受保护文件工作区/` — 文档启用了「限制编辑」且自动解保护失败时，抽取出来的可对比副本，便于人工检查。
- `summary.html` — 总览页：A/B 扫描数、可配对数、仅 A / 仅 B、有差异 / 无差异 / 失败的详细中文清单。
...
- **未接受修订自动接受**：如果 A 或 B 的 .docx 里含有未接受的修订，v2.0 会在对比前自动 `AcceptAllRevisions` 再走 `CompareDocuments`，并在结果里标注「已自动接受 N 处修订」。
- **限制编辑保护**：先尝试 `Unprotect()`；失败则把内容抽取到一份新 .docx 放进 `受保护文件工作区/<相对路径>/` 后再对比，并在结果里给出中文提示。
- 真正受密码保护、损坏、或被其他程序独占的文件仍会被标记「失败/跳过」，理由写在汇总里。
